...
6. ~~No **Preferred Communication** field has been identified.~~ **DONE 2026-08-13.** The field is `registrations.f2993`, alias "Prefered Communication (Email" (sic — truncated in Ontraport), type `list`, options `398=Both, 399=Email, 400=Call`. It was already recorded in `ontraport-setup-punchlist.md` — this item was stale, not undiscovered. Now in `Roster Fetch`'s `listFields` and rendered in the participant details drawer via `rosterListLabels()` / `ROSTER_PREFERRED_COMM_MAP`.

   **`list` is not `drop`.** A `list` field does not return a bare option ID. It comes back delimiter-wrapped — `*/*398*/*` for one value, `*/*399*/*400*/*` for several — confirmed by writing and reading back registration 1164, not inferred. `rosterListLabels()` decodes both, drops unrecognised IDs, and returns `''` for empty/`"0"` so the caller renders its own em dash.

   **⚠️ Every populated record says the same thing.** The field went from 0 populated records to 171 during the evening of 2026-08-13. All 171 hold `398` "Both" — **zero** Email, **zero** Call — and on event 218 all 138 registrations are set with none empty. The drawer therefore shows "Both" for every participant. That is the genuine stored value and is reported as-is, but a perfectly uniform value across an entire roster is what a bulk default looks like, not what 138 individually-expressed preferences look like. Worth confirming how it was set before a CS relies on it to choose how to contact someone — this is the same trap the original fabricated "Email" default came from.

   `qvPreferredComm` in the Home roster popout is deliberately still an em dash — see item 4; that popout has no registration binding to read from.
