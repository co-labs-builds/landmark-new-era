...
1. **🚨 `CS Dashboard : Roster Fetch` (`PlbMTP01FSvTe2s9`) doesn't request the participant contact/Information Form fields.** Add to its **Get Registrations** node's `listFields`: `f3252`, `f3253`, `f2574`, `f2575`, `f2576`, `f2578`, `f2579`, `f2580`, `f2582`, `f2583`, `f2584`, `f2586`, `f2676` (`f2585` is already there). Email/phone/emergency-contact render as em dashes until this lands — the client side is already done and lights up the moment it does. `f3252`/`f3253` are registration-level **mirror** fields of the Contact's email/`sms_number`, so no extern hop is needed. The node's `listFields` is ~50 entries and `updateNodeParameters` needs `replace:true` with the full parameter block; silently dropping one entry breaks roster rendering non-obviously, so verify by executing against `eventTeamId` 7 and confirming the returned key set is a strict superset of the previous one.
...
6. No **Preferred Communication** field has been identified on the registration or contact. That row shows an em dash rather than asserting a value.
